...
export function partnerHardeningPlan({ readiness, productionReadiness, hardening, idp, publicIdentity, monitor, pilotProof = null, sessionDrill = null }) {
  return {
    ...
    lanes: [
      ...
      { id: 'partner_pilot_proof', status: pilotProof?.ok === false ? 'pending' : 'done', evidence: pilotProof?.evidence_hash || '/api/ops/partner-pilot-proof' },
      ...
      { id: 'oidc_user_login', status: idp.implementation_state === 'oidc_configured_fail_closed' ? 'done' : 'pending', evidence: idp.implementation_state },
      { id: 'two_tenant_browser_isolation', status: sessionDrill?.ok ? 'done' : 'pending', evidence: sessionDrill?.evidence_hash || 'requires real two-browser/customer-session drill' },
...
function verifierSummary(receipt) {
  return {
    ...
    value_released: receipt.verified === true && receipt.status === 'verified',

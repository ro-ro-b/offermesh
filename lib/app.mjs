...
// v0.10.0: production-readiness evidence over the exact supplied UI and unchanged write boundaries.
...
import { monitor, readiness, hardening } from './ops.mjs';
import { billingPolicy, dualLiveReadbackPlan, idpAccessContract, marketPack } from './contracts.mjs';
import { customerSessionDrill, incidentRunbook, productionReadiness, publicIdentity } from './production.mjs';
import { resolveOidcSession } from './idp.mjs';
...
import {
  ...
  brandDashboard,
  createOfferFlow,
  partnerHardeningPlan,
  proofRoom,
  proofRoomHtml,
  referenceAgentGuide,
  runPartnerDemo
} from './productSurfaces.mjs';
...
export async function handle(req, res) {
  ...
  try {
    if (req.method === 'POST' && url.pathname === '/mcp') {
      const rl = await rateLimit(req, 'mcp');
...
async function api(req, res, url) {
  ...
  const rl = await rateLimit(req, cls);
  if (!rl.allowed) return send(res, 429, { error: 'rate_limited', retry_after_seconds: rl.retry_after_seconds });
  ...
  if (method === 'GET' && p === '/api/ops/hardening') return send(res, 200, hardening(store));
  if (method === 'GET' && p === '/api/ops/production-readiness') return send(res, 200, productionReadiness(store, monitor(store)));
  if (method === 'GET' && p === '/api/ops/partner-pilot-proof') return send(res, 200, partnerPilotProof({ baseUrl: originFromReq(req), version: VERSION }));
  if (method === 'GET' && p === '/api/ops/incident-runbook') return send(res, 200, incidentRunbook());
  if (method === 'GET' && p === '/api/ops/customer-session-drill') return send(res, 200, customerSessionDrill(store));
  if (method === 'GET' && p === '/api/ops/idp-contract') return send(res, 200, idpAccessContract());
  if (method === 'GET' && p === '/api/ops/billing-policy') return send(res, 200, billingPolicy());
  if (method === 'GET' && p === '/api/dual/live-readback-plan') return send(res, 200, dualLiveReadbackPlan());
  if (method === 'GET' && p === '/api/product/market-pack') return send(res, 200, marketPack());
  if (method === 'GET' && p === '/api/product/public-identity') return send(res, 200, publicIdentity());
  if (method === 'GET' && p === '/api/product/agent-marketplace') return send(res, 200, agentMarketplace(store, { category: url.searchParams.get('category') || '' }));
  if (method === 'GET' && p === '/api/product/brand-dashboard') return send(res, 200, brandDashboard(store, { brandRef: url.searchParams.get('brandRef') || '' }));
  if (method === 'GET' && p === '/api/product/reference-agent') return send(res, 200, referenceAgentGuide());
  if (method === 'GET' && /^\/api\/product\/proof-room\/[^/]+$/.test(p)) return send(res, 200, proofRoom(store, decodeURIComponent(p.split('/')[4]), originFromReq(req)));
  if (method === 'GET' && p === '/api/ops/partner-hardening') {
    const mon = monitor(store);
    const prod = productionReadiness(store, mon);
    return send(res, 200, partnerHardeningPlan({
  ...
  if (method === 'GET' && p === '/api/auth/session') {
    const session = await resolveOidcSession(store, req, 'member');
    if (!session.ok) return send(res, session.httpStatus || 401, { status: session.status, detail: 'OIDC session not established; tenant API keys remain available.' });
    return send(res, 200, { status: 'authenticated', auth_type: session.auth_type, tenant: publicTenant(session.tenant), subject: session.subject, email: session.email, roles: session.roles });
  }
  ...
  if (p.startsWith('/api/tenant/')) {
    ...
    if (!access) return send(res, 401, tenantAuthRequired());
  ...
  // ---- guided partner product flows ----
  if (method === 'POST' && p === '/api/product/create-offer-flow') {
    const access = await resolveTenantAccess(req);
    if (!access) return send(res, 401, tenantAuthRequired());
    if (access.tenant.blocked) return send(res, 403, { status: 'tenant_suspended' });
    const b = await readJson(req);
    return guard(res, () => {
      const out = createOfferFlow(store, access.tenant.id, b, originFromReq(req));
      meter(store, access.tenant.id, 'guided_offers_created');
      meter(store, access.tenant.id, 'programs_created');
      meter(store, access.tenant.id, 'offers_minted');
      return sendSave(res, 201, out);
    });
  }
  if (method === 'POST' && p === '/api/product/partner-demo') {
    const access = await resolveTenantAccess(req);
    if (!access) return send(res, 401, tenantAuthRequired());
    ...
    const b = await readJson(req);
    return guard(res, () => {
      const out = runPartnerDemo(store, access.tenant.id, b, originFromReq(req));
      meter(store, access.tenant.id, 'partner_demo_runs');
      meter(store, access.tenant.id, 'verified_outcomes');
      meter(store, access.tenant.id, 'redemptions');
      return sendSave(res, 201, out);
    });
  }
...
function tenantAuthRequired() {
  return { status: 'tenant_auth_required', detail: 'Tenant API key required (x-offermesh-tenant-key), or valid OIDC bearer token when OIDC env is configured. Fails closed.' };
}
async function resolveTenantAccess(req) {
  const tenant = resolveTenantByApiKey(store, req.headers['x-offermesh-tenant-key']);
  if (tenant) return { tenant, auth_type: 'tenant_api_key' };
  const session = await resolveOidcSession(store, req, 'member');
...
function setSecurityHeaders(res) {
  ...
  res.setHeader('content-security-policy', "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com data:; connect-src 'self'; img-src 'self' data:; frame-ancestors 'none'");
...
async function serveStatic(req, res, url) {
  if (url.pathname.startsWith('/proof/')) {
    ...
    res.end(proofRoomHtml(id));
    return;
  }
  const path = ['/', '/revolv', '/revolv/'].includes(url.pathname) ? '/index.html' : url.pathname;
...
function originFromReq(req) {
  ...
  const proto = req.headers['x-forwarded-proto'] || (/^(localhost|127\.0\.0\.1|\[::1\])(?::|$)/.test(host) ? 'http' : 'https');
  return `${proto}://${host}`;
}

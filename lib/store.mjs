...
export function createStore() {
  const store = { statePath: null, remote: null };
...
export function persistRemote(store) {
  ...
  pendingSave = (async () => {
    ...
    pendingSave = null;
    try { await store.remote.save(snapshot(store)); store.lastRemoteSave = new Date().toISOString(); }
...
export function persist(store) {
  ...
  writeFileSync(store.statePath, JSON.stringify(snapshot(store)));
  return true;
}

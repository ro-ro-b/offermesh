...
export function readiness(store) {
  ...
  return {
    ...
    items: [
      ...
      item('durable_storage', store.remote ? 'done' : 'pending', store.remote ? `${store.remote.kind}; ${concurrency.mode}` : 'Upstash Redis not bound in this environment'),
      ...
      item('two_tenant_session_drill', sessionDrill.ok ? 'done' : 'pending', sessionDrill.ok ? `Replayable two-tenant session drill ready (${sessionDrill.passed}/${sessionDrill.passed + sessionDrill.failed}, ${sessionDrill.evidence_hash})` : 'Replayable tenant-session isolation evidence missing'),
      ...
      item('broad_production_review_gate', broadReview.passed ? 'done' : 'pending', broadReview.passed ? `External Claude Cowork returned ${broadReview.score}/10 for ${broadReview.claim_scope}` : 'Fresh broad production/partner-ready Cowork review required for this v0.10.0 tranche'),
      item('multi_region_concurrency', concurrency.protected ? 'done' : 'pending', concurrency.protected ? concurrency.detail : 'Snapshot persistence is last-writer-wins; Redis optimistic locking or Postgres required for contended scale')
    ],
...
export function hardening(store) {
  ...
  return {
    ...
    storage: {
      ...
      concurrency: storageConcurrencyStatus(store)
    },
    idp: oidcStatus(),

export function idpAccessContract() {
  return {
    status: 'phase_2_contract_ready',
    provider: 'bring_your_own_oidc',
    implementation_state: 'not_bound',
    product: 'revolv',
    engine: 'offermesh',
    guarantees: [
    ...
    blocked_until_bound: ['browser_user_login', 'self_serve_invites', 'role_admin_ui'],
    next_step: 'Bind Auth0/Clerk/WorkOS OIDC only after the external review gate or Ian approval'
  };
}
...
export function billingPolicy() {
  return {
    ...
    next_step: 'Choose Stripe sandbox, invoice-only, or platform rev-share after legal and Cowork review'
  };
}
...
export function marketPack() {
  return {
    ...
    caveats: [
      'Ungated until external Claude Cowork returns 9.8 or Ian overrides',
...
export function hardeningContract({ globalRateLimit = false } = {}) {
  return {
    ...
    next_hardening: ['external alert sink', 'IdP role binding', 'fine-grained datastore concurrency', 'Cowork review']
  };
}

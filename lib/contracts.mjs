...
export function idpAccessContract() {
  ...
  return {
    ...
    runtime: status,
    next_step: status.implementation_state === 'oidc_configured_fail_closed' ? 'Run two-browser tenant isolation drill and broad Cowork production review' : 'Bind Auth0/Clerk/WorkOS OIDC env values, then run two-browser tenant isolation drill'
  };
}
...
export function marketPack() {
  return {
    ...
    caveats: [
      'Scoped external Claude Cowork review returned 9.8/10 for v0.4.0 next-six; broad production-ready review is still pending',

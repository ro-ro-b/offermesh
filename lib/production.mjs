...
import { publicUrls, COWORK_SCOPED_REVIEW } from './version.mjs';
...
export function productionReadiness(store, monitorResult) {
  ...
  const items = [
    ...
    item('broad_production_cowork_review', 'pending', 'Fresh broad production/partner-ready Cowork review must run against this exact v0.5.x commit/deployment'),
    ...
    item('two_tenant_browser_isolation', 'pending', 'Needs two real browser sessions or replayable session drill evidence after IdP/public URL decision'),
    item('durable_storage', durable ? 'done' : 'pending', durable ? `${store.remote.kind}; ${fineGrained ? 'fine-grained/atomic mode asserted by env' : 'whole-snapshot last-writer-wins'}` : 'No hosted durable storage adapter bound'),
    item('fine_grained_concurrency', fineGrained ? 'done' : 'pending', fineGrained ? `OFFERMESH_STORAGE_CONCURRENCY_MODE=${process.env.OFFERMESH_STORAGE_CONCURRENCY_MODE}` : 'Current persistence is snapshot-level; use Postgres or fine-grained Redis before contended multi-tenant scale'),
  ...
  const blockers = items.filter((i) => ['pending', 'partial'].includes(i.status)).map((i) => i.id);
  return {
    product: 'revolv',
    engine: 'offermesh',
    state: blockers.length === 0 ? 'production_ready_candidate' : 'production_pilot_incomplete',
    production_ready_claim_allowed: blockers.length === 0,
    partner_ready_claim_allowed: false,
...
export function customerSessionDrill(store) {
  return {
    status: 'ready_to_run_after_idp_or_two_browser_contexts',
    ...
    gate: 'Must be captured before broad production-ready or partner-ready claim'
  };
}

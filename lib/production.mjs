...
import { publicUrls, COWORK_SCOPED_REVIEW, broadCoworkReview } from './version.mjs';
...
export function productionReadiness(store, monitorResult) {
  ...
  const items = [
    ...
    item('broad_production_cowork_review', broadReview.passed ? 'done' : 'pending', broadReview.passed ? `External Cowork returned ${broadReview.score}/10 for ${broadReview.scope}` : 'Fresh broad production/partner-ready Cowork review must run against this exact v0.9.0 commit/deployment'),
...
export function customerSessionDrill(store) {
  return {
    ...
    gate: 'Must be captured before full production-ready claim; partner-ready pilot may proceed with tenant-key isolation if broad Cowork explicitly accepts that scope'
  };
}

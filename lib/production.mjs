...
export function productionReadiness(store, monitorResult) {
  ...
  const items = [
    ...
    item('broad_production_cowork_review', broadReview.passed ? 'done' : 'pending', broadReview.passed ? `External Cowork returned ${broadReview.score}/10 for ${broadReview.scope}` : 'Fresh broad production/partner-ready Cowork review must run against this exact v0.5.x commit/deployment'),

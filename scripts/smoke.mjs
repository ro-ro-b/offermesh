// Full-loop REST smoke against a spawned server instance — v0.10.0 hardening proof included.
...
try {
  ...
  assert('status v0.10.0', status.body.version === '0.10.0', status.body.version);
  ...
  // v0.10.0 product surfaces, partner-pilot proof, and session isolation drill
  ...
  assert('partner hardening plan separates done from pending gates', partnerHardening.body.lanes.some((lane) => lane.id === 'guided_offer_creation' && lane.status === 'done') && partnerHardening.body.lanes.some((lane) => lane.id === 'partner_pilot_proof' && lane.status === 'done') && partnerHardening.body.lanes.some((lane) => lane.id === 'two_tenant_browser_isolation' && lane.status === 'done') && partnerHardening.body.partner_ready_claim_allowed === false);
  ...
  const sessionDrillEarly = await get('/api/ops/customer-session-drill');
  assert('customer session drill is replayable and bounded', sessionDrillEarly.body.status === 'customer_session_drill_ready' && sessionDrillEarly.body.summary.failed === 0 && sessionDrillEarly.body.evidence.cross_tenant_denials.every((d) => d.http_status === 403), sessionDrillEarly.body);
  ...
  assert('readiness: two-tenant session drill done', ids.two_tenant_session_drill === 'done');
  ...
  assert('production readiness records session isolation drill', prod.body.items.some((i) => i.id === 'two_tenant_browser_isolation' && i.status === 'done') && !prod.body.blockers.includes('two_tenant_browser_isolation'), prod.body.blockers);
  ...
  assert('customer session drill is explicit', drill.body.status === 'customer_session_drill_ready' && drill.body.required_evidence.length >= 4);
